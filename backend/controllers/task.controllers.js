...
const getUserTasks = async (req, res) => {
  try {
    ...
    tasks.forEach((task) => {
      if (task.status === "assigned") tasksData.assigned.push(task.title);
      if (task.status === "ongoing") tasksData.ongoing.push(task.title);
      if (task.status === "completed") tasksData.completed.push(task.title);
    });
    ...
    console.log("Authenticated User:", JSON.stringify(userInfo, null, 2));

    res.status(200).json({ user: userInfo, tasks });
  } catch (error) {
...
const addSubTask = async (req, res) => {
  try {
    ...
    const { title, description, status, priority, dueDate, assignedToEmail } =
      req.body;
...
const updateSubTask = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Sub Task updated successfully.", subTask });
  } catch (error) {
